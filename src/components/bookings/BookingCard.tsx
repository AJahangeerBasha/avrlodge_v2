...
import { Phone, User, Hash, Calendar, Users, Home, DollarSign, X, AlertTriangle, History, FileText, Eye, ChevronDown, ChevronUp, RefreshCw, Receipt, Settings, Edit3 } from 'lucide-react'
...
export default function BookingCard({
  ...
  const loadDocuments = useCallback(async () => {
    try {
      ...
      console.error('BookingCard: Error loading documents:', error)
      toast({
        title: "Error",
  ...
  const loadSpecialCharges = useCallback(async () => {
    try {
      ...
      let charges: ReservationSpecialCharge[] = []
      try {
  ...
  const loadPrimaryGuest = useCallback(async () => {
    try {
      ...
      if (guest) {
        setGuestForm({
          name: guest.name || booking.guest_name || '',
          phone: guest.phone || booking.guest_phone || '',
          whatsapp: guest.whatsapp || '',
          usePhoneForWhatsapp: !guest.whatsapp || guest.whatsapp === guest.phone
        })
      } else {
        // No guest record found, use booking data
        setGuestForm({
  ...
  // Handle primary guest edit modal
  const handleOpenPrimaryGuestEdit = () => {
    loadPrimaryGuest()
    setShowPrimaryGuestEdit(true)
  ...
  const handleUpdatePrimaryGuest = async () => {
    ...
    try {
      ...
      // Note: If no guest record exists, we could create one here if needed
