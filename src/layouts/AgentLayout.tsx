...
export function AgentLayout() {
  ...
  // Redirect if not agent, manager or admin (higher roles can access agent panel)
  ...
  const navigationItems = [
    { to: "/agent", icon: BarChart3, label: "Dashboard" },
    { to: "/agent/calendar", icon: Calendar, label: "Calendar" },
    { to: "/agent/reservation", icon: BookOpen, label: "Reservations" },
    { to: "/agent/bookings", icon: Users, label: "Bookings" }
  ]
      ...
      <div className="bg-white shadow">
        <div className="px-4 py-4 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <div>
              <h1 className="text-lg md:text-xl font-semibold text-gray-900">AVR Lodge Agent</h1>
              {(userRole === ROLES.ADMIN || userRole === ROLES.MANAGER) && (
                <p className="text-xs md:text-sm text-gray-500">{userRole === ROLES.ADMIN ? 'Admin' : 'Manager'} accessing as Agent</p>
              )}
            </div>
          </div>
        ...
        <div className="md:hidden bg-black text-white">
          <div className="px-4 py-4 space-y-2">
            ...
            <div className="pb-4 border-b border-gray-700">
              ...
              {(userRole === ROLES.ADMIN || userRole === ROLES.MANAGER) && (
                <div className="text-xs text-gray-400">{userRole === ROLES.ADMIN ? 'Admin' : 'Manager'} accessing as Agent</div>
              )}
            </div>
            ...
            {userRole === ROLES.MANAGER && (
              <div className="pb-2">
                ...
                >
                  <Settings className="h-5 w-5" />
                  <span className="text-base">Go to Manager Panel</span>
                </Button>
              </div>
            )}
